...
export default class ServicesList extends Component{
    ...
    render(){
        ...
        switch(this.state.selectedTab){
            case 'Offering':offeringColor='#000';break;
            case 'Requests':requestsColor='#000';break;
        }
        return(
            <View style={styles.container}>
                <View style={styles.top}>
                    <Text style={{color:'white',fontSize:16,fontWeight:'bold'}}>Services</Text>
                    <Image style={styles.roundImage}></Image>
                </View>
                <View style={styles.subContainer}>
                   <Text style={{fontWeight:'bold'}}>Community Name</Text>
                   <View style={{flexDirection:'row',marginTop:20}}>
                        <TouchableOpacity style={{flex:1,alignItems:'center'}} onPress={()=>this.setState({selectedTab:'Offering'})}>
                            <Text style={{color:offeringColor}}>Offering</Text>
                            <View style={[styles.line,{backgroundColor:offeringColor}]}></View>    
                        </TouchableOpacity>
                        <TouchableOpacity style={{flex:1,alignItems:'center'}} onPress={()=>this.setState({selectedTab:'Requests'})}>
                            <Text style={{color:requestsColor}}>Requests</Text>
                                ...
                                <View key={key} style={{paddingTop:20}}>
                                    ...
                                    <View style={{flexDirection:'row',marginTop:10,alignItems:'center',justifyContent:'space-between'}}>
                                        <View style={{flexDirection:'row'}}>
                                            ...
                                            <Text style={{color:'#707070',marginLeft:10}}> Chat</Text>
                                        </View>
                                        <TouchableOpacity style={{borderRadius:20,padding:5,paddingHorizontal:20,backgroundColor:'#0D9EDD'}}>
